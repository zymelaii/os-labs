...
static u32 sys_fork() { return fork(NULL); }

static u32 sys_fork_ack() {
  fork_ack();
  return 0;
}

static u32 sys_exec() { return exec(NULL, SYSCALL_ARGS1(const char *)); }

static u32 sys_wait() { return wait(SYSCALL_ARGS1(int *)); }

static u32 sys_exit() {
  exit(NULL, SYSCALL_ARGS1(int));
  return 0;
}

static syscall_t SYSCALL_TABLE[NR_SYSCALLS] = {
    SYSCALL_ENTRY(get_ticks), SYSCALL_ENTRY(get_pid), SYSCALL_ENTRY(read),
    SYSCALL_ENTRY(write),     SYSCALL_ENTRY(fork),    SYSCALL_ENTRY(fork_ack),
    SYSCALL_ENTRY(exec),      SYSCALL_ENTRY(wait),    SYSCALL_ENTRY(exit),
};
